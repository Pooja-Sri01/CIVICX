"""
CIVICX Database Seed Data
Realistic Coimbatore Municipal Infrastructure Dataset (60+ Assets)
Annotated with geo-coordinates, damage telemetry, computer vision detections, and maintenance records.
"""

from typing import List, Dict, Any

COIMBATORE_ASSETS: List[Dict[str, Any]] = [
    {
        "id": "civicx-ast-001",
        "assetId": "RD-1042",
...
# Generate additional assets to bring dataset to realistic demo scale (80 assets total)
ZONE_COORDINATE_ANCHORS = {
    "Central Zone": (11.012, 76.965),
    "East Zone": (11.020, 77.010),
    "West Zone": (11.015, 76.935),
    "North Zone": (11.055, 76.975),
    "South Zone": (10.975, 76.970)
}

ASSET_TYPES = ["Road", "Bridge", "Drainage", "Culvert", "Flyover", "Traffic Corridor"]

def get_all_seed_assets() -> List[Dict[str, Any]]:
    import random
    random.seed(42) # Deterministic generation for reproducible demo results
    
    assets = list(COIMBATORE_ASSETS)
    
    # Generate 62 more assets deterministically
    names_templates = [
        ("Vadavalli Thondamuthur Link Road", "Road", "West Zone", "Ward 15", 11.008, 76.912),
        ("Singanallur - Vellalore Bypass Culvert", "Culvert", "South Zone", "Ward 80", 10.965, 77.012),
        ("Kavundampalayam MTP Flyover Pier", "Flyover", "North Zone", "Ward 10", 11.045, 76.948),
        ("Sivananda Colony Railway Feeder", "Road", "Central Zone", "Ward 20", 11.028, 76.958),
        ("Ganapathy Maniyakarampalayam Road", "Road", "North Zone", "Ward 16", 11.048, 76.985),
        ("Sowripalayam Meena Estate Cross Link", "Road", "East Zone", "Ward 45", 11.005, 77.001),
        ("Ramanathapuram 80 Feet Road Culvert", "Culvert", "South Zone", "Ward 65", 10.982, 76.988),
        ("Ondipudur Flyover Trichy Road Entry", "Flyover", "East Zone", "Ward 55", 10.995, 77.042),
        ("Vilankurichi Tech Park Transit Corridor", "Traffic Corridor", "East Zone", "Ward 35", 11.052, 77.018),
        ("Sundarapuram Madukkarai Link Road", "Road", "South Zone", "Ward 92", 10.942, 76.968),
        ("Perur Pateeswarar Temple North Arch Road", "Road", "West Zone", "Ward 73", 10.985, 76.920),
        ("Avarampalayam Railway Level Cross Road", "Road", "Central Zone", "Ward 26", 11.029, 76.980),
        ("Sulur Airforce Station Access Road", "Road", "East Zone", "Ward 50", 11.018, 77.085),
        ("Cheran Ma Nagar Drainage Main Interceptor", "Drainage", "East Zone", "Ward 37", 11.045, 77.012),
        ("Kovaipudur Valley Arterial Road", "Road", "South Zone", "Ward 88", 10.938, 76.935),
        ("Pappanaickenpalayam Mani High School Rd", "Road", "Central Zone", "Ward 30", 11.018, 76.978),
        ("Hope College Underpass Arterial", "Road", "East Zone", "Ward 40", 11.028, 77.014),
        ("Gopalapuram Club Road", "Road", "Central Zone", "Ward 27", 11.009, 76.971),
        ("Kuniyamuthur Kovaipudur Link Culvert", "Culvert", "South Zone", "Ward 86", 10.952, 76.945),
        ("Chinnavedampatti Lake Canal", "Drainage", "North Zone", "Ward 08", 11.065, 76.988)
    ]
    
    idx = 19
    for i, (name, atype, zone, ward, base_lat, base_lng) in enumerate(names_templates):
        # Deterministic variation
        cond = int(25 + ((i * 17) % 65))
        severity = 100 - cond + random.randint(-5, 5)
        severity = max(5, min(98, severity))
        
        crit_score = int(40 + ((i * 23) % 55))
        usage_score = int(35 + ((i * 19) % 60))
        trend_score = int(30 + ((i * 29) % 60))
        exp_score = int(25 + ((i * 31) % 65))
        
        # Risk score calculation
        risk = int(0.25 * (100 - cond) + 0.25 * severity + 0.20 * crit_score + 0.15 * usage_score + 0.10 * trend_score + 0.05 * exp_score)
        risk = max(5, min(99, risk))
        
        risk_level = "Critical" if risk >= 76 else "High" if risk >= 51 else "Medium" if risk >= 26 else "Low"
        crit_level = "Critical" if crit_score >= 80 else "High" if crit_score >= 60 else "Medium" if crit_score >= 40 else "Low"
        
        cost = int((100000 + (100 - cond) * 28000 + crit_score * 12000) / 10000) * 10000
        
        prefix = "RD" if atype == "Road" else "BR" if atype == "Bridge" else "DR" if atype == "Drainage" else "CU" if atype == "Culvert" else "FL" if atype == "Flyover" else "TC"
        
        asset_obj = {
            "id": f"civicx-ast-{idx:03d}",
            "assetId": f"{prefix}-{1000 + idx * 7}",
            "name": name,
            "type": atype,
            "location": f"{name}, {zone}",
            "ward": ward,
            "zone": zone,
            "latitude": round(base_lat + (random.random() - 0.5) * 0.015, 4),
            "longitude": round(base_lng + (random.random() - 0.5) * 0.015, 4),
            "conditionScore": cond,
            "damageSeverity": severity,
            "damageType": "Pavement Fatigue Cracking & Localized Raveling" if atype == "Road" else "Joint Seal Wear & Surface Spalling" if atype == "Bridge" else "Sedimentation & Invert Degradation",
            "riskScore": risk,
            "riskLevel": risk_level,
            "criticality": crit_level,
            "criticalityScore": crit_score,
            "usage": f"Urban Feeder Corridor ({int(15000 + usage_score * 350)} PCU/day)",
            "usageScore": usage_score,
            "historicalTrend": f"Degradation rate +{int(10 + trend_score * 0.25)}%/yr",
            "trendScore": trend_score,
            "environmentalExposure": "Monsoon Inundation & Traffic Load",
            "exposureScore": exp_score,
            "estimatedRepairCost": cost,
            "priorityRank": idx,
            "recommendedAction": "Preventative Resurfacing & Base Stabilization" if risk >= 50 else "Routine Inspection & Sealant",
            "image": "https://images.unsplash.com/photo-1518609878373-06d740f60d8b?auto=format&fit=crop&w=1000&q=80" if atype == "Road" else "https://images.unsplash.com/photo-1513836279014-a89f7a76ae86?auto=format&fit=crop&w=1000&q=80" if atype == "Bridge" else "https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?auto=format&fit=crop&w=1000&q=80",
            "detectedBBoxes": [
                {"label": "Surface Distress (D20)", "confidence": 0.88, "x": 30, "y": 40, "width": 40, "height": 30}
            ] if risk >= 50 else [],
            "lastInspection": "2026-07-25",
            "maintenanceHistory": [
                {"date": "2024-06-15", "action": "Routine Surface Dressing", "cost": 150000, "vendor": "Municipal Maintenance Corps", "conditionAfter": 80}
            ],
            "explainability": {
                "summary": f"Asset evaluated in {zone} showing {risk_level.lower()} risk based on {usage_score} usage density and {cond}% condition integrity.",
                "topFactors": [
                    {"factor": "Structural Condition Deficit", "impact": "High" if cond < 40 else "Moderate", "weight": 0.25, "scoreContribution": round(0.25 * (100 - cond), 1), "description": f"Condition score evaluated at {cond}/100."},
                    {"factor": "Usage & Traffic Density", "impact": "High" if usage_score > 70 else "Moderate", "weight": 0.15, "scoreContribution": round(0.15 * usage_score, 1), "description": f"Carrying estimated {int(15000 + usage_score * 350)} PCU daily."}
                ],
                "whyRank": f"Assigned priority rank based on multi-criteria value maximization.",
                "preventativeROI": f"{round(2.5 + (risk / 30.0), 1)}x ROI"
            }
        }
        assets.append(asset_obj)
        idx += 1
        
    # Recalculate priority ranks strictly by Priority Scoring formula
    for asset in assets:
        # Priority formula: (Risk * Criticality * Usage) / (Cost^0.35)
        r = asset["riskScore"]
        k = asset["criticalityScore"]
        u = asset["usageScore"]
        c = max(100000, asset["estimatedRepairCost"])
        priority_metric = (r * 1.5 + k * 1.2 + u * 0.8) / (c ** 0.15)
        asset["_priorityMetric"] = priority_metric
        
    assets.sort(key=lambda x: x["_priorityMetric"], reverse=True)
    for rank, asset in enumerate(assets, 1):
        asset["priorityRank"] = rank
        del asset["_priorityMetric"]
