...
export const LoginPage: React.FC = () => {
  ...
  // Municipal form state
  const [muniEmail, setMuniEmail] = useState('');
  const [muniPassword, setMuniPassword] = useState('');
  ...
  const triggerToast = (text: string) => {
    setAutoFilledToast(text);
    setTimeout(() => setAutoFilledToast(null), 2500);
  };
  ...
  // ============================================================
  // SINGLE AUTO-FILL HANDLERS
  // ============================================================
  const handleAutoFillMunicipal = () => {
    setMuniRole('CHIEF_ENGINEER');
    setMuniEmail('authority@coimbatore.gov.in');
    setMuniPassword('civicx2026');
    setError(null);
    triggerToast('Municipal credentials loaded');
  };

  const handleAutoFillCitizen = () => {
    setCitEmail('priya.sundaram@gmail.com');
    setCitPassword('citizen123');
    setError(null);
    triggerToast('Citizen credentials loaded');
  };
  ...
  const handleMunicipalSubmit = async (e: React.FormEvent) => {
    ...
    if (!muniEmail.trim() || !muniPassword) {
      setError('Please enter municipal email and access key, or click Auto-Fill.');
      return;
    }
  ...
  const handleCitizenSignInSubmit = async (e: React.FormEvent) => {
    ...
    if (!citEmail.trim() || !citPassword) {
      setError('Please enter citizen email and password, or click Auto-Fill.');
      return;
    }
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-canvas flex items-center justify-center p-4 sm:p-6 lg:p-8 font-sans">
      <div className="w-full max-w-4xl rounded-3xl bg-white border border-zinc-200 shadow-elevated overflow-hidden grid grid-cols-1 lg:grid-cols-12">
        ...
        <div className="lg:col-span-5 bg-zinc-950 text-white p-8 sm:p-10 flex flex-col justify-between relative overflow-hidden">
          <div className="space-y-6 relative z-10">
            ...
            <div className="space-y-3 pt-6 border-t border-white/10">
              ...
              <p className="text-xs text-zinc-400 leading-relaxed font-normal">
                Unified intelligence platform connecting Coimbatore citizens and municipal engineers for data-driven infrastructure maintenance.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-8 border-t border-white/10 relative z-10 text-xs font-mono">
            <div className="flex items-center gap-2 text-zinc-300">
              ...
              <span>Citizen Civic Rewards & Reports</span>
            </div>
            <div className="flex items-center gap-2 text-zinc-300">
              <CheckCircle2 className="w-3.5 h-3.5 text-lime" />
            ...
            <div className="flex items-center gap-2 text-zinc-300">
              <CheckCircle2 className="w-3.5 h-3.5 text-lime" />
              <span>10-Step Municipal Decision Engine</span>
            </div>
          </div>
        ...
        <div className="lg:col-span-7 p-6 sm:p-10 flex flex-col justify-between space-y-6">
          <div className="space-y-5">
            ...
            {autoFilledToast && (
              <div className="py-2 px-3 rounded-xl bg-lime/15 border border-lime/40 text-civic-dark text-xs font-mono font-bold flex items-center justify-between animate-fadeIn">
                <span className="flex items-center gap-1.5">
                  ...
                  <span>{autoFilledToast}</span>
                </span>
                <span className="text-[10px] uppercase tracking-wider text-zinc-500">Ready to Sign In</span>
              </div>
            )}
            ...
            {/* ============================================================ */}
            {/* A. MUNICIPAL OFFICIAL EXPERIENCE                             */}
            {/* ============================================================ */}
            {authMode === 'MUNICIPAL' && (
              <form onSubmit={handleMunicipalSubmit} className="space-y-4">
                {/* Single Clean Auto-Fill Button for Municipal */}
                <div className="flex items-center justify-between p-3 rounded-xl bg-zinc-50 border border-zinc-200">
                  <div>
                    <span className="text-xs font-bold text-civic-dark font-mono block">
                      Evaluation / Demo Login
                    </span>
                    <span className="text-[11px] text-zinc-500">
                      Chief Engineer (<code className="text-zinc-700">authority@coimbatore.gov.in</code>)
                    </span>
                  </div>
                  <button
                    type="button"
                    onClick={handleAutoFillMunicipal}
                    className="px-3 py-1.5 rounded-lg bg-lime hover:bg-lime-light text-civic-dark text-xs font-mono font-bold transition-all flex items-center gap-1.5 shadow-sm"
                  >
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="INSPECTOR">Senior Infrastructure Inspector (Field Audits)</option>
                  </select>
                </div>
            ...
            {/* ============================================================ */}
            {/* B. CITIZEN PORTAL EXPERIENCE                                 */}
            {/* ============================================================ */}
            {authMode === 'CITIZEN' && (
              <div className="space-y-4">
                {/* Single Clean Auto-Fill Button for Citizen */}
                <div className="flex items-center justify-between p-3 rounded-xl bg-zinc-50 border border-zinc-200">
                  <div>
                    <span className="text-xs font-bold text-civic-dark font-mono block">
                      Evaluation / Demo Citizen
                    </span>
                    <span className="text-[11px] text-zinc-500">
                      Priya Sundaram (<code className="text-zinc-700">priya.sundaram@gmail.com</code>)
                    </span>
                  </div>
                  <button
                    type="button"
                  ...
                  <form onSubmit={handleVerifyOtpSubmit} className="space-y-4">
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-mono text-zinc-400">Verification Code</span>
                      </div>
                      <h3 className="font-display font-black text-base text-civic-dark">
                        Check Your Email Inbox
                      </h3>
                      <p className="text-xs text-zinc-500 font-medium">
                        Code sent to <strong className="text-civic-dark">{citEmail}</strong>.
                      </p>
                    </div>
                    ...
                    {devOtpHint && (
                      <button
                        ...
                        className="w-full py-2 px-3 rounded-xl bg-zinc-900 text-lime text-xs font-mono font-bold hover:bg-zinc-800 transition-all flex items-center justify-center gap-1.5"
                      >
                        <Zap className="w-3.5 h-3.5" />
                        <span>Auto-Fill Code ({devOtpHint})</span>
                      </button>
                    )}
                ...
                {citizenStep === 'REGISTER_STEP_3_PASS' && (
                  <form onSubmit={handleCompleteRegistrationSubmit} className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <label className="text-xs font-bold text-zinc-700 font-mono">MOBILE (OPTIONAL)</label>
                        <input
                          type="tel"
                          value={citPhone}
                      ...
                      <div className="space-y-1">
                        <label className="text-xs font-bold text-zinc-700 font-mono">WARD</label>
                        <select
                          value={citWard}
                ...
                {citizenStep === 'REGISTER_STEP_4_SUCCESS' && (
                  <div className="text-center py-6 space-y-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-zinc-500 font-medium">
                        Your CIVICX Citizen account is active with 100 Welcome Points.
                      </p>
                    </div>
